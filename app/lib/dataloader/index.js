...
function stringifyObject(
  object,
  keys = [...Object.keys(object), ...Object.getOwnPropertySymbols(object)]
) {
  return keys
    .sort((lhs, rhs) => {
    ...
    .map((key) => `${key.toString()}:${stringifyValue(object[key], key)}`)
    .join("|")
}
...
function loaderForBTM(
  ...
  assert(
    ...
    "options.include is not supported by model loader"
  )
  assert(
    options.association !== undefined,
    "options.association should be set for BTM loader"
  )
  ...
  return new DataLoader(
    (keys) => {
      ...
      if (findOptions.limit) {
        const limit =
          findOptions.offset && findOptions.offset > 0
            ? [findOptions.limit, findOptions.offset]
            : findOptions.limit
      ...
      return model
        .findAll(findOptions)
        .then(mapResult.bind(null, attributes, keys, findOptions))
    },
    {
      cache: options.cache
    }
...
function shimModel(target) {
  if (target.findByPk ? target.findByPk.__wrapped : target.findById.__wrapped)
    return
  ...
  shimmer.massWrap(target, methods(Sequelize.version).findByPk, (original) => {
    return function batchedFindById(id, options = {}) {
      if (
        options.transaction ||
        options.include ||
        activeClsTransaction() ||
        !options[EXPECTED_OPTIONS_KEY]
      ) {
      ...
      return Promise.resolve()
        .then(() => {
          ...
          if (options.raw || options.paranoid === false) {
            const cacheKey = getCacheKey(this, this.primaryKeyAttribute, {
              raw: options.raw,
              paranoid: options.paranoid
            })
            loader = loaders.autogenerated.get(cacheKey)
            if (!loader) {
              loader = createModelAttributeLoader(
                this,
    ...
    return function batchedGetBelongsTo(instance, options = {}) {
      ...
      return Promise.resolve()
        .then(() => {
          ...
          const loaders = options[EXPECTED_OPTIONS_KEY].loaders
          let loader =
            loaders[this.target.name].bySingleAttribute[this.targetKey]
          if (options.raw || options.paranoid === false) {
            const cacheKey = getCacheKey(this.target, this.targetKey, {
              raw: options.raw,
              paranoid: options.paranoid
            })
            loader = loaders.autogenerated.get(cacheKey)
            if (!loader) {
              loader = createModelAttributeLoader(this.target, this.targetKey, {
                raw: options.raw,
                paranoid: options.paranoid,
                logging: options.logging
              })
    ...
    return function batchedGetHasOne(instance, options = {}) {
      ...
      return Promise.resolve()
        .then(() => {
          ...
          const loaders = options[EXPECTED_OPTIONS_KEY].loaders
          let loader =
            loaders[this.target.name].bySingleAttribute[this.foreignKey]
          if (options.raw || options.paranoid === false) {
            const cacheKey = getCacheKey(this.target, this.foreignKey, {
              raw: options.raw,
              paranoid: options.paranoid
            })
            loader = loaders.autogenerated.get(cacheKey)
  ...
  shimmer.wrap(target, "get", (original) => {
    return function batchedGetHasMany(instances, options = {}) {
      ...
      if (!loader) {
        ...
        options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.set(
          cacheKey,
          loader
        )
      }
      ...
      if (Array.isArray(instances)) {
        ...
        return Promise.resolve(loader.load(sourceKeyValue))
          .then((result) => {
            if (isCount && !result) {
              result = { count: 0 }
            }
...
function shimBelongsToMany(target) {
  ...
  shimmer.wrap(target, "get", (original) => {
    return function batchedGetBelongsToMany(instances, options = {}) {
      ...
      assert(
        this.paired,
        ".paired missing on belongsToMany association. You need to set up both sides of the association"
      )
      ...
      if (
        ...
        options.multiple = false
        options.group = [
          `${this.paired.manyFromSource.as}.${this.identifierField}`
        ]
        delete options.plain
        isCount = true
      }
      ...
      const cacheKey = getCacheKey(
        this.target,
        [this.paired.manyFromSource.as, this.foreignKey],
        loaderOptions
      )
      loader = options[EXPECTED_OPTIONS_KEY].loaders.autogenerated.get(cacheKey)
      if (!loader) {
      ...
      if (Array.isArray(instances)) {
        return Promise.map(instances, (instance) =>
          loader.load(instance.get(this.source.primaryKeyAttribute))
        )
      } else {
        return Promise.resolve(
          loader.load(instances.get(this.source.primaryKeyAttribute))
        )
          .then((result) => {
            if (isCount && !result) {
              result = { count: 0 }
...
export function createContext(sequelize, options = {}) {
  ...
  shimModel(
    versionTestRegEx.test(sequelize.constructor.version) // v3 vs v4
      ? sequelize.constructor.Model
      : sequelize.constructor.Model.prototype
  )
  ...
  loaders.autogenerated = new LRUCache({ max: options.max || 500 })
  ...
  for (const Model of Object.values(sequelize.models)) {
    ...
    loaders[Model.name].bySingleAttribute[Model.primaryKeyAttribute] =
      createModelAttributeLoader(Model, Model.primaryKeyAttribute, options)
    loaders[Model.name].byId = loaders[Model.name].byPrimaryKey =
      loaders[Model.name].bySingleAttribute[Model.primaryKeyAttribute]
  }
    ...
    values(Model.associations).forEach((association) => {
      if (association.associationType === "BelongsTo") {
        ...
        if (association.targetKey !== Target.primaryKeyAttribute) {
          loaders[Target.name].bySingleAttribute[association.targetKey] =
            createModelAttributeLoader(Target, association.targetKey, options)
        }
      } else if (association.associationType === "HasOne") {
        const Target = association.target
        loaders[Target.name].bySingleAttribute[association.foreignKey] =
          createModelAttributeLoader(Target, association.foreignKey, options)
      }
    })
  }
  ...
  function prime(results) {
    ...
    results.forEach((result) => {
      ...
      Object.keys(loaders[modelName].bySingleAttribute).forEach((attribute) => {
        loaders[modelName].bySingleAttribute[attribute].prime(
...
export function removeContext(sequelize) {
  ...
  shimmer.unwrap(
    sequelize.constructor.Association.BelongsToMany.prototype,
    "get"
  )
}
