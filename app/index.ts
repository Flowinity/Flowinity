...
async function initTPU() {
    ...
    const mainWorker: boolean = !cluster.worker || cluster.worker?.id % cpuCount === 1
...
async function buildFrontend() {
    ...
    try {
        ...
        if (
            !(fs.existsSync(path.join(global.rawAppRoot, "../frontend_build")))
        ...
        console.error(`[PRIVATEUPLOADER/FRONTEND] Failed to build with error: ${err}.`)
    }
}
