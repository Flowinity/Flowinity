...
export class Server {
    ...
    async init(port?: number, noBackgroundTasks: boolean = false): Promise<void> {
        ...
        if (config.finishedSetup) {
            ...
            global.domain = await Domain.findOne({
                where: {id: 1}
            }).then((domain: Domain | null): string | undefined => {
        ...
        process.on(
            "uncaughtException",
            function (err: Error, origin: NodeJS.UncaughtExceptionOrigin): void {
                console.log(origin)
                console.warn(err)
            }
        )
        ...
        if (mainWorker && !noBackgroundTasks) {
            ...
            console.info("[PRIVATEUPLOADER] Background tasks not started due to non-primary worker.")
        }
    }
    ...
    private onError(error: NodeJS.ErrnoException): void {
        ...
        switch (error.code) {
            ...
            case "EADDRINUSE":
                ...
                console.error(`[PRIVATEUPLOADER] ${bind} is already in use`)
    ...
    private onListening(): void {
        ...
        console.info(`[PRIVATEUPLOADER] Listening on ${bind}`)
    }
}
