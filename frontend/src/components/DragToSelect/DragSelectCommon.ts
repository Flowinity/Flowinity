...
export type InnerDragSelectProps<I extends DragSelectProps, T> = {
  readonly [P in RequiredKeys<I>]-?: P extends keyof DragSelectProps
    ? NonNullable<DragSelectProps<T>[P]>
    : never;
} & {
  readonly [P in OptionalKeys<I>]?: P extends keyof DragSelectProps
    ? DragSelectProps<T>[P]
    : never;
};
