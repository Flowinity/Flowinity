...
const tableGridSize = reactive<GridSize>({
  rows: display.mobile.value ? TABLE_MAX_GRID_SIZE : TABLE_INIT_GRID_SIZE,
  cols: display.mobile.value ? TABLE_MAX_GRID_SIZE : TABLE_INIT_GRID_SIZE
});
...
<template>
  ...
  >
    <VCard density="compact" class="table-grid-size-editor">
      ...
      <VCardText
        class="d-flex flex-column flex-wrap justify-space-between pa-2"
      >
