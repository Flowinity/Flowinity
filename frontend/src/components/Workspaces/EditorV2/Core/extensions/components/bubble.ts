...
import type {
  ...
  ButtonViewReturn,
  ExtensionNameKeys
} from "@/components/Workspaces/EditorV2/Core/types";
...
const imageFloatMenus = (editor: Editor): BubbleMenuItem[] => {
  const types: BubbleImageFloatType[] = [
    "float-left",
    "float-none",
    "float-right"
  ];
...
const imageSizeMenus = (editor: Editor): BubbleMenuItem[] => {
  ...
  return types.map((size, i) => ({
    ...
    componentProps: {
      ...
      icon: icons[i],
      action: () =>
        editor.commands.updateImage({ width: IMAGE_SIZE[size], height: null }),
    ...
    for (const ext of nodeType) {
      ...
      if (find) {
        _items.push({
          ...
          componentProps: {
            ...find.componentProps,
            tooltip: find.componentProps.tooltip
              ? t(find.componentProps.tooltip)
              : undefined
          },
