...
const display = computed<ImageAttrsOptions["display"]>(
  () => props.node.attrs.display || undefined
);
const lockAspectRatio = computed<ImageAttrsOptions["lockAspectRatio"]>(
  () => props.node.attrs.lockAspectRatio ?? true
);
...
const onMouseMove = throttle(function (e: MouseEvent) {
  ...
  const height = unref(lockAspectRatio)
    ? null
    : Math.max(h + dy, IMAGE_MIN_SIZE);
